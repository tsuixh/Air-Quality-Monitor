...
void setup() {
  ...
  attachInterrupt(interruptPin, updateScreen, CHANGE);
  //初始化预热指示灯
...
double getDustDensity(double outputV) {
  ...
  double ugm3 = outputV * 200 - 200 + 61.7;
...
void tftSetup() {
  ...
  screen.text("HCOH(ppm):", 5, 75);
  screen.text("AQI:", 5, 95);
...
void tftUpdate() {
  ...
  //甲醛
  screen.text(ch4, 65, 70);
  //dew
  screen.text(aqi, 85, 90);
  //suggest
...
void updateScreen() {
  ...
  if (isCh4Ready) {
    tempVal = String((float)tCh4,7);
    tempVal.toCharArray(ch4, 7);
  }
  tftUpdate();
}
...
void dataInit() {
  ...
  val.toCharArray(aqi, 6);
  val.toCharArray(suggest, 8);
  val = String("wait");
  val.toCharArray(ch4, 8);
}
